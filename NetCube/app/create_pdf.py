...
# Combinar la ruta del archivo con la ruta de la imagen
img_path = os.path.join(dir_path, 'Cubo.png')
...
def render_table_header(pdf):
    ...
    for col_name in TABLE_COL_NAMES:
        if col_name == "MENSAJE":
            pdf.cell(pdf.epw / 2.5, line_height, col_name, border=1)
        else:
            if col_name == "IP":
                ...
                if col_name == "NIVEL":
                    ...
                    pdf.cell(pdf.epw / 8, line_height, col_name, border=1)
...
def create_pdf(values):
    ...
    pdf.image(img_path, 93, w = 20, h = 20)
    ...
    for row in values:
        ...
        for datum in row:
            if i == 5:
                ...
                pdf.cell(pdf.epw / 2.5 , line_height, str(datum), border=1)
            else:
                if i == 4:
                    ...
                    if i == 3:
                        ...
                        pdf.cell(pdf.epw / 8 , line_height, str(datum), border=1)
            i += 1
        pdf.ln(line_height)
